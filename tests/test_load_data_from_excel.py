...
def test_load_data_from_excel_success():
    ...
    test_data = {
        ...
        "Дополнительный_столбец": ["X", "Y", "Z"]
    }
    mock_dataframe = pd.DataFrame(test_data)

    with patch('os.path.exists', return_value=True), \
        patch('pandas.read_excel', return_value=mock_dataframe):
...
def test_load_data_from_excel_file_not_found():
    ...
    with patch('os.path.exists', return_value=False):
        with pytest.raises(ValueError,
                           match="Ошибка при сохранении результата: Файл не найден: non_existent_file.xlsx"):
...
def test_load_data_from_excel_replace_nan():
    ...
    test_data = {
        "Категория": [None, "Транспорт", "Развлечения"],
        "Сумма операции": [100, None, 20]
    }
    mock_dataframe = pd.DataFrame(test_data)
