...
def test_main_success(mock_get_stock_price, mock_get_currency_rates, mock_load_data_from_excel):
    # Мокируем загрузку данных из Excel, чтобы возвращать фиктивные данные
    ...
    # Мокируем get_currency_rates, чтобы возвращать фиктивные данные
    mock_get_currency_rates.return_value = [
        {"валюта": "USD", "ставка": 75.0},
        {"валюта": "EUR", "ставка": 85.0}
    ]

    # Мокируем get_stock_price, чтобы возвращать фиктивные данные
    mock_get_stock_price.return_value = {"AAPL": 150.0, "AMZN": 3000.0}

    result = main("2023-01-01", "M")

    assert isinstance(result, dict)
    assert "Диапазон дат" in result
    assert "Результат анализа" in result
    assert "Курсы валют" in result
    assert "Биржевые цены" in result
...
def test_main_json_save_error(mock_env_vars, mock_1_load_data_from_excel, mock_analyze_transactions, mock_get_currency_rates, mock_get_stock_price):
...
def test_main_stock_api_error(mock_stock_price, mock_currency_rates, mock_env_vars, mock_1_load_data_from_excel, mock_analyze_transactions):
...
def test_main_range_type_all():
    ...
    with patch('src.views.load_data_from_excel', return_value=[
        ...
        assert result['Диапазон дат']['Начало'] == "0001-01-01"  # Минимальная дата
        assert result['Диапазон дат']['Конец'] == "2023-01-01"
